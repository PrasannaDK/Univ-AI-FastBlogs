...
class ClfCallback(AccCallback):
    def __init__(self, learner, bs, dataSet):
    ...
    def epoch_end(self):
        super().epoch_end()
        validateTrain = np.mean((self.learner.model(self.X_train) >= 0.5)*1 == self.y_train)
        validateTest = np.mean((self.learner.model(self.X_test) >= 0.5)*1 == self.y_test)
        self.accuracies.append(validateTrain)
        self.test_accuracies.append(validateTest)
        if self.epoch % 10 ==0:
            print(f"Epoch {self.epoch} Train-Accuracy {validateTrain} Test-Accuracy {validateTest}")
