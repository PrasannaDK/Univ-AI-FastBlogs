...
class Affine(Layer):
    ...
    def __call__(self, inputs):
        self.inputs = inputs
        #print(self.name,'w', self.params['w'].shape, 'b', self.params['b'].shape,'inp', self.inputs.shape)
        return inputs@self.params['w'] + self.params['b']

    def backward(self, grad):
        # (m,n) @ (n,  1) = (m, 1)
        #print("gradshape",self.name, grad.shape)
        self.grads['w'] = self.inputs.T @ grad
        # (n, 1) @ (n, 1) = (1,)
        self.grads['b'] = np.sum(grad, axis=0)
        # return (n, 1) @ (1 , m) = (n, m)
        #print('params-w',self.params['w'].shape)
...
class Activation(Layer):
    ...
    def backward(self, grad):
        # (N,1) * (N,1) = (N,1)
        #print('activation', self.name, self.inputs.shape, grad.shape, self.f_prime(self.inputs).shape)
